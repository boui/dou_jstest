var growlModule = angular.module('ui.growl', ['template/growl/box.html']);

growlModule.controller('GrowlController', ['$scope', '$timeout', 'growl', 'model', function ($scope, $timeout, growl, model) {
    var currentTimeout;
    ...
    this.$get = ["$http", "$document", "$compile", "$rootScope", "$controller", "$templateCache", "$q", "$injector", function ($http, $document, $compile, $rootScope, $controller, $templateCache, $q, $injector) {
        ...
        return {
            ...
            box: function (title, text, options) {
                ...
                options = angular.extend(options, {uuid: Math.random()});
